...
async function execute() {
	while (true) {
		...
		console.log(`List of commands: create, read, update, delete, autogeneration, search, exit`);
		...
		if (input === 'read') {
			...
			while (true) {
				const reqNumber = await readline.Question('Enter the request number: ');
				if (reqNumber === '1') {
					const itemName = await readline.Question(`Enter author's name: `);
					const enteredAge = await readline.Question(`Enter author's age span(e.g. 4-29): `);
					const itemAge = {};
					[itemAge.lower, itemAge.upper] = enteredAge.split('-');
					const itemTitle = await readline.Question(`Enter news title: `);
					const timeBeforeReq = Date.now();
					await basicController.searchFirstReq(itemName, itemAge, itemTitle);
					const timeAfterReq = Date.now();
					console.log(`Time taken: ${timeAfterReq - timeBeforeReq}ms`);
					break;
				} else if (reqNumber === '2') {
					const enteredItemId = await readline.Question(`Enter news id span(e.g. 4-29): `);
					const itemId = {};
					[itemId.lower, itemId.upper] = enteredItemId.split('-');
					const itemName = await readline.Question(`Enter category name: `);
					const enteredItemDate = await readline.Question(`Enter news publication date span(e.g. 2010-10-10/2019-05-11): `);
					const itemDate = {};
					[itemDate.lower, itemDate.upper] = enteredItemDate.split('/');

					const timeBeforeReq = Date.now();
					await basicController.searchSecondReq(itemId, itemName, itemDate);
					const timeAfterReq = Date.now();
					console.log(`Time taken: ${timeAfterReq - timeBeforeReq}ms`);
					break;
				} else if (reqNumber === '3') {
					const enteredNewsId = await readline.Question(`Enter news id span(e.g. 4-29): `);
					const newsId = {};
					[newsId.lower, newsId.upper] = enteredNewsId.split('-');
					const enteredAuthorId = await readline.Question(`Enter author id span(e.g. 4-29): `);
					const authorId = {};
					[authorId.lower, authorId.upper] = enteredAuthorId.split('-');
					const categoryName = await readline.Question(`Enter category name: `);

					const timeBeforeReq = Date.now();
					await basicController.searchThirdReq(newsId, authorId, categoryName);
					const timeAfterReq = Date.now();
					console.log(`Time taken: ${timeAfterReq - timeBeforeReq}ms`);
					break;
