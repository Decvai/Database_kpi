...
module.exports = {
	...
	searchFirstReq: async function (itemName, itemAge, itemTitle) {
		try {
			const rows = await basicModel.searchFirstReq(itemName, itemAge, itemTitle);
			basicView.showItems(rows);
			return rows;
		} catch (err) {
			console.log(err);
	...
	searchSecondReq: async function (itemId, itemName, itemDate) {
		try {
			const rows = await basicModel.searchSecondReq(itemId, itemName, itemDate);
			basicView.showItems(rows);
			return rows;
		} catch (err) {
			console.log(err);
	...
	searchThirdReq: async function (newsId, authorId, categoryName) {
		try {
			const rows = await basicModel.searchThirdReq(newsId, authorId, categoryName);
			basicView.showItems(rows);
			return rows;
		} catch (err) {
			console.log(err);
	...
	checkOnSubordinates: async function (subordinateTableName, foreignKeyField, itemId) {
		try {
			return await basicModel.checkOnSubordinates(subordinateTableName, foreignKeyField, itemId);
		} catch (err) {
			console.log(err);
		}
	},

	sortByLikes: async function (category, countdownDate) {
		try {
			const rows = await basicModel.sortByLikes(category, countdownDate);
			basicView.showItems(rows);
			return rows;
		} catch (err) {
			console.log(err);
	...
	sortByViews: async function (category, countdownDate) {
		try {
			const rows = await basicModel.sortByViews(category, countdownDate);
			basicView.showItems(rows);
			return rows;
		} catch (err) {
			console.log(err);
